...
class AppController(QtCore.QObject):
    ...
    def _process_actions(self):
        while True:
            ...
            if action == "toggle_rest_mode":
                ...
                self.ui.hide_calibration_overlay()
                self.last_action = "calibration reset"
            elif action == "save_settings":
                self._save_settings()
                self.last_action = "settings saved"
    ...
    def _start_calibration(self):
        ...
        self.last_action = "calibration started"
    ...
    def _handle_calibration(self, tracking_result, blink_info):
        ...
        event = self.calibration_manager.observe(
            {
                "normalized_point": np.asarray(tracking_result["normalized_point"], dtype=np.float32),
        ...
        if event["status"] == "completed":
            self.ui.hide_calibration_overlay()
            self.cursor_mapper.set_calibration(event["calibration"])
