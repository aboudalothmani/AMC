...
class CalibrationManager:
    ...
    def observe(self, sample):
        ...
        if len(self.current_samples) >= samples_per_point:
            ...
            min_effective = max(8, samples_per_point // 2)
            if aggregated["effective_samples"] < min_effective:
                self.current_samples = [sample for sample, keep in zip(self.current_samples, aggregated["inlier_mask"]) if keep]
                aggregated = None
                self.last_hint = "Noise detected, collecting a cleaner lock"
        ...
        self.current_samples = []
        self.recent_points = []
        self.last_hint = "Move to the next target"
    ...
    def _finalize(self):
        ...
        best_fit = min(fits, key=lambda fit: (fit["cross_validation_error_px"], fit["mean_error_px"]))
    ...
    def _build_hint(self, settled, stable, pose_ok, quality_ok, blink_triggered, quality, stability):
        if not settled:
            return "Hold still while the new target settles"
        if blink_triggered:
            return "Keep eyes open during calibration sampling"
        if not quality_ok:
            return f"Move closer or improve lighting, quality {quality:.2f}"
        if not pose_ok:
            return "Center your head and reduce tilt"
        if not stable:
            if stability is None:
                return "Stabilizing gaze lock"
            return f"Stabilizing gaze lock, jitter {stability:.4f}"
        return "Locked, sampling this point"
